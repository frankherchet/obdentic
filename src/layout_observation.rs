...
impl Default for LayoutFreshnessPolicy {
    fn default() -> Self {
        Self {
            ...
            sparkline: Duration::from_millis(500),
...
/// Explicit live orchestration seam: derive semantic layout demand and pass it
/// through the existing budget/support policy. The returned plan exposes the
/// requested interval, effective interval, status, and reason for each signal.
...
mod tests {
    ...
    fn duplicate_layout_demand_merges_only_in_subscription_policy() {
        ...
        let rpm = plan
            ...
            .unwrap();
        assert_eq!(rpm.requested_interval(), Duration::from_millis(500));
